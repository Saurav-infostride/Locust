...
class MyScript(SequentialTaskSet):
    ...
    @task
    def get_xml(self):
        result = self.client.get("/xml", name="XML")  # storing the response in result
        print(result)
    ...
    def get_json(self):
        ...
        with self.client.get("/json", catch_response=True, name="JSON") as response:  # Name of response is JSON
